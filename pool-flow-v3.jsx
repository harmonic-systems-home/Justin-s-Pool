...
export default function PoolSystemV3() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const st = await window.storage.get("pool-v3:state");
        if (st?.value) setS((p) => ({ ...p, ...JSON.parse(st.value) }));
      } catch {}
      try {
        const n = await window.storage.get("pool-v3:notes");
  ...
  useEffect(() => {
    ...
    (async () => { try { await window.storage.set("pool-v3:state", JSON.stringify(s)); } catch {} })();
  }, [s]);
  const saveNotes = async (v) => { setNotes(v); try { await window.storage.set("pool-v3:notes", v); } catch {} };
