...
### 9. Add the spillway to the schematic (spillover config, 7/26)
The SPA and POOL nodes are adjacent — add a short **SPILLWAY edge directly between them** (spa → pool), representing the designed overflow weir. This is a GRAVITY edge, not pumped plumbing, so style it distinctly: shorter/wider than pipes, cascade-like (thin parallel falling dashes or a weir glyph), labeled "spillway."
**Activation logic:** flowing whenever the spa receives more than it holds — pump running AND return valve delivering any spa fraction (split or full-spa return) AND suction NOT on spa. In the normal spillover config (return split, suction pool) it animates during every pump run, so the schematic's resting state shows the true daily water path: pump → filter → heater → pad valve → deck return valve → spa jets → **over the weir** → pool. Silent spillway with pump running = the anomaly state (matches the care-calendar glance).
Color follows temperature: blue normally; red during heat runs (the spillway is how split-return heating reaches the pool via the spa).

### 10. Split DECK PAIR into two independent valve nodes (spillover config, 7/26)
The two in-ground valves have independent positions in the normal config, so one merged node misrepresents the system. Replace with two tappable valves:
- **SUCTION VALVE** — left sleeve, nearer the equipment pad (EST — verify by thermal touch test, test 5). Normal position: **full POOL**. Positions: POOL / SPA (rotating to SPA = spa-heating sessions only).
- **RETURN VALVE** — right sleeve, nearer the spa; carries the ←FLOW embossment (EST — same verification). Normal position: **SPLIT** (partial flow to spa jets, rest to pool floor returns; excess spills back via the weir — see #9). Positions: POOL / SPLIT / SPA.
**Naming standard (enforce in all UI text): "SUCTION" (draws to pump) and "RETURN" (delivers to spa jets) — never "supply," which has now been used ambiguously in both directions.** Each valve node shows its sleeve location ("left/near-pad", "right/near-spa") so a person standing at the open sleeves knows which hole is which. Normal-day preset: suction=POOL, return=SPLIT; heat-spa preset: both=SPA; flow solver + spillway (#9) activation follow accordingly.
