...
- **Deck valve pair — SPILLOVER configuration (corrected 7/26, per Justin):** two in-ground valves near the spa; handles parallel to side of house = POOL, 180° = SPA. **Daily config: RETURN valve split (partial heated/filtered flow to spa), SUCTION valve full POOL. The spa has a DESIGNED OVERFLOW CHANNEL (spillway/weir) into the pool — excess return flow spills back, so the spa level is pinned at the weir, no suction/return balancing is ever needed, and the spa can't "steal" pool water.** Spa gets continuous turnover on every pump run. Full SPA mode (both valves rotated 180°) is used only for spa-heating sessions. Spillway flowing during a pump run = free visual health indicator; a silent spillway mid-run is the anomaly. **Sleeve identification (EST, per geometry + the ←FLOW embossment; verify by thermal touch test): LEFT sleeve (nearer equipment pad) = SUCTION valve, normally full POOL; RIGHT sleeve (nearer spa) = RETURN valve, normally SPLIT. Terminology standard: SUCTION / RETURN only — never "supply" (ambiguous).**
...
- **HAZARD (revised 7/26 for spillover config):** with daily suction on full POOL, diverting the pad valve to WATERFALL is SAFE — the spa just stops spilling over and sits at its weir. The drain-down hazard exists only in **full SPA mode (suction on spa) + waterfall** — pumps the spa out through the waterfall. No documented procedure produces that state; rule remains: when heating the spa, pad valve stays on POOL.
